...
function JobList() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      setLoading(true);
      try {
        const params = { page, limit, search: search.trim(), category, showArchived };
  ...
  if (loading) return <div className="flex justify-center items-center h-64 text-gray-500">იტვირთება...</div>;
      ...
      {jobs.length === 0 ? (
        <div className="text-center py-12 sm:py-20 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
          ...
          {user?.role === 'client' && !showArchived && (
            <Link to="/post-job" className="text-indigo-600 hover:underline mt-2 inline-block text-sm">
              + დაამატეთ პირველი დავალება
            </Link>
          )}
          {!user && (
            <Link to="/login" className="text-indigo-600 hover:underline mt-2 inline-block text-sm">
              გაიარეთ ავტორიზაცია
            </Link>
          )}
        </div>
            ...
            <Link key={job._id} to={`/jobs/${job._id}`} className="block transform transition hover:scale-[1.02] duration-200">
              <div className="bg-white rounded-xl shadow-md hover:shadow-xl border border-gray-100 overflow-hidden h-full flex flex-col">
                <div className="px-4 sm:px-5 pt-3 sm:pt-4 pb-1 sm:pb-2 flex justify-between items-start">
                  ...
                  <span className={`text-xs font-semibold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full ${
                ...
                <div className="px-4 sm:px-5 py-1 sm:py-2 flex-grow">
                  <h2 className="text-lg sm:text-xl font-bold text-gray-800 line-clamp-1">{job.title}</h2>
                  <p className="text-gray-500 text-xs sm:text-sm mt-1 line-clamp-2">{job.description}</p>
                </div>
                <div className="px-4 sm:px-5 py-3 sm:py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                  <span className="text-gray-600 text-xs sm:text-sm">📍 {job.district}</span>
                  <span className="text-base sm:text-lg font-bold text-green-600">
                    {job.budget} <span className="text-xs sm:text-sm font-normal text-gray-500">GEL</span>
                  </span>
                </div>
              </div>
            </Link>
